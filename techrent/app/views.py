...
from .models import Transaction, User, Device, Authenticator, Recommendation
...
def createDevice(request):
    if(request.method == "POST"):
        try:
            ...
            new_device.save()
            return HttpResponse(str(new_device.id) + ' ' + str(NewOwner))
...
def getUserFromAuth(request):
	auth = request.GET['auth']
	user = Authenticator.objects.get(authenticator=auth).user_id
	return HttpResponse(user)

def updateRecs(request):
	Recommendation.objects.all().delete()
	output = json.loads(request.POST.get('output'))
	for item in output:
		thing = item[0]
		recs = " ".join(item[1])
		try:
			new_rec = Recommendation(item_id=thing, recommended_items=recs)
			new_rec.save()
		except Exception as e:
			return HttpResponse(e)
	return HttpResponse("Database Recommendations created/updated")

	
def getRecs(request, id):
	try:
		rec = Recommendation.objects.get(item_id=id).recommended_items
